<template>
  <div class="space-y-6">
    ...
    <p v-if="!backendModeEnabled" class="text-center text-sm text-gray-500 dark:text-dark-400">
...
type AuthFormMode = 'page' | 'drawer'

const props = withDefaults(
  defineProps<{
    mode?: AuthFormMode
    consumeExpiredSession?: boolean
  }>(),
  {
    mode: 'page',
    consumeExpiredSession: false
  }
)

const emit = defineEmits<{
  (event: 'switch-mode', mode: 'login' | 'register'): void
  (event: 'success'): void
}>()
...
onMounted(async () => {
  if (props.consumeExpiredSession) {
