...
var usageLogInsertArgTypes = [...]string{
	...
	"text",        // route_mode
	"text",        // route_mapping_rule
	"integer",     // route_attempt_count
	"jsonb",       // route_failures
...
func prepareUsageLogInsert(log *service.UsageLog) usageLogInsertPrepared {
	...
	routeMappingRule := ""
	if log.RouteMappingRule != nil {
		routeMappingRule = *log.RouteMappingRule
	}
	routeAudit := (&service.RouteAudit{
		Mode:        log.RouteMode,
		MappingRule: routeMappingRule,
		Attempts:    log.RouteAttemptCount,
		Failures:    log.RouteFailures,
	}).Snapshot()
	log.RouteMode = routeAudit.Mode
	log.RouteAttemptCount = routeAudit.Attempts
	log.RouteFailures = routeAudit.Failures
	log.RouteMappingRule = optionalRouteMappingRule(routeAudit.MappingRule)
	routeFailures := routeFailuresJSON(routeAudit.Failures)
	...
	return usageLogInsertPrepared{
		...
		args: []any{
			...
			routeAudit.Mode,
			nullString(log.RouteMappingRule),
			routeAudit.Attempts,
			routeFailures,
			createdAt,
		},
...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		routeMode             sql.NullString
		routeMappingRule      sql.NullString
		routeAttemptCount     sql.NullInt64
		routeFailures         sql.NullString
		createdAt             time.Time
	)
	...
	routeAudit := (&service.RouteAudit{
		...
		MappingRule: routeMappingRule.String,
		Attempts:    int(routeAttemptCount.Int64),
		Failures:    routeFailuresFromNullJSON(routeFailures),
	}).Snapshot()
	log := &service.UsageLog{
		ID:                    id,
		UserID:                userID,
		...
		RouteMappingRule:      optionalRouteMappingRule(routeAudit.MappingRule),
		RouteAttemptCount:     routeAudit.Attempts,
		RouteFailures:         routeAudit.Failures,
		CreatedAt:             createdAt,
	}
...
func optionalRouteMappingRule(value string) *string {
...
func routeFailuresJSON(failures []service.RouteFailureEntry) string {
	if failures == nil {
		failures = []service.RouteFailureEntry{}
	}
...
func routeFailuresFromNullJSON(value sql.NullString) []service.RouteFailureEntry {
	failures := []service.RouteFailureEntry{}
	if !value.Valid || strings.TrimSpace(value.String) == "" {
		return failures
	}
	if err := json.Unmarshal([]byte(value.String), &failures); err != nil || failures == nil {
		return []service.RouteFailureEntry{}
	}
	return failures
}
